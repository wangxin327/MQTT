...
int net__socket_connect_tls(struct mosquitto *mosq)
{
	...
	fprintf(stderr,"SSL connection using %s, %s\n", SSL_get_version(mosq->ssl), SSL_get_cipher(mosq->ssl));
	//ShowCerts(mosq->ssl);
	return MOSQ_ERR_SUCCESS;
}
#endif
...
#endif
*/
			mosq->ssl_ctx = SSL_CTX_new(CNTLS_client_method());
		...
		//SSL_CTX_set_cipher_list(mosq->ssl_ctx, "ECC-SM4-SM3");
		SSL_CTX_clear_options(mosq->ssl_ctx, SSL_OP_ENABLE_MIDDLEBOX_COMPAT);
...
#  endif
			...
			if(mosq->tls_cert_reqs == 0){
				...
				//SSL_CTX_set_verify(mosq->ssl_ctx, SSL_VERIFY_PEER|SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NULL);
				SSL_CTX_set_verify_depth(mosq->ssl_ctx, 1);
			}
			...
			if(mosq->tls_keyfile){
				...
				if(ret != 1){
					log__printf(mosq, MOSQ_LOG_ERR, "Error1: Client certificate/key are inconsistent.");
#if !defined(OPENSSL_NO_ENGINE)
					ENGINE_FINISH(engine);
#endif
					net__print_ssl_error(mosq);
		...
		if(mosq->tls_enc_certfile){
				ret = SSL_CTX_use_enc_certificate_file(mosq->ssl_ctx, mosq->tls_enc_certfile, SSL_FILETYPE_PEM);
				if(ret != 1){
#ifdef WITH_BROKER
					log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load client enc certificate, check bridge_certfile \"%s\".", mosq->tls_enc_certfile);
#else
					log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load client enc certificate \"%s\".", mosq->tls_enc_certfile);
#endif
#if !defined(OPENSSL_NO_ENGINE)
					ENGINE_FINISH(engine);
#endif
			...
			if(mosq->tls_enc_keyfile){
				if(mosq->tls_keyform == mosq_k_engine){
					...
					pkey = ENGINE_load_private_key(engine, mosq->tls_enc_keyfile, ui_method, NULL);
					if(!pkey){
						log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load engine private key file \"%s\".", mosq->tls_keyfile);
						ENGINE_FINISH(engine);
						net__print_ssl_error(mosq);
					...
					if(SSL_CTX_use_enc_PrivateKey(mosq->ssl_ctx, pkey) <= 0){
						log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to use engine private key file \"%s\".", mosq->tls_keyfile);
						ENGINE_FINISH(engine);
						net__print_ssl_error(mosq);
						return MOSQ_ERR_TLS;
					...
					ret = SSL_CTX_use_enc_PrivateKey_file(mosq->ssl_ctx, mosq->tls_enc_keyfile, SSL_FILETYPE_PEM);
					if(ret != 1){
#ifdef WITH_BROKER
						log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load client enc_key file, check bridge_keyfile \"%s\".", mosq->tls_enc_keyfile);
#else
						log__printf(mosq, MOSQ_LOG_ERR, "Error: Unable to load client key file \"%s\".", mosq->tls_enc_keyfile);
#endif
#if !defined(OPENSSL_NO_ENGINE)
						ENGINE_FINISH(engine);
#endif
				...
				ret = SSL_CTX_check_enc_private_key(mosq->ssl_ctx);
				if(ret != 1){
					log__printf(mosq, MOSQ_LOG_ERR, "Error2: Client certificate/key are inconsistent.");
#if !defined(OPENSSL_NO_ENGINE)
					ENGINE_FINISH(engine);
#endif
					net__print_ssl_error(mosq);
