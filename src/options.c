...
int mosquitto_tls_set(struct mosquitto *mosq, const char *cafile, const char *capath, const char *certfile, const char *keyfile, const char *enc_certfile, const char *enc_keyfile,int (*pw_callback)(char *buf, int size, int rwflag, void *userdata))
{
#ifdef WITH_TLS
	FILE *fptr;
	...
	/****************/
	if(enc_certfile){
		fptr = mosquitto__fopen(enc_certfile, "rt", false);
		if(fptr){
			fclose(fptr);
		}else{
			mosquitto__free(mosq->tls_cafile);
		...
		mosq->tls_enc_certfile = mosquitto__strdup(enc_certfile);
	...
	mosq->tls_enc_keyfile = NULL;
	if(enc_keyfile){
		fptr = mosquitto__fopen(enc_keyfile, "rt", false);
		if(fptr){
			fclose(fptr);
		}else{
			mosquitto__free(mosq->tls_cafile);
			...
			mosq->tls_enc_certfile = NULL;
			return MOSQ_ERR_INVAL;
		}
		mosq->tls_enc_keyfile = mosquitto__strdup(enc_keyfile);
